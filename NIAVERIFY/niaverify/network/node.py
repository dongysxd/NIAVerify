...
from niaverify.common.utils import ReluState, ReluApproximation
...
class Node:
    ...
    def __init__(
        ...
        self.biases = None
        self.biases_sum = None
        self.relax_diff = None
        self.impact = None
        self.non_lin_indices = None
        self.origin_non_lin_indices = None
        self.first_calc_bounds_flag = True
...
#a
    def _set_relaxation_for_node(self):
        bound = 'lower'
        lower_slope = self.get_relu_slope('lower', bound)
        upper_slope = self.get_relu_slope('upper', bound)
        #out_flag = None if out_flag is None else out_flag.flatten()
        #lower_const = self.derive_const()
        ...
        upper_const = lower_const.clone()
        idxs = self.get_unstable_flag().flatten()
        upper_const[idxs] = -upper_slope[idxs] * self.from_node[0].bounds.lower.flatten()[idxs]
        ##a
        self.relaxations = torch.zeros((2,  self.input_size, 2), dtype=self.config.PRECISION).to( device=self.config.DEVICE)

        # node.to_node[0].relaxations[0,:,0] = lower_slope
        # node.to_node[0].relaxations[1,:,0] = upper_const
        # node.to_node[0].relaxations[0,:,1] = lower_const
        # node.to_node[0].relaxations[1,:,1] = upper_const

        # node.to_node[0].input_size
        self.relaxations[0,:,0] = lower_slope
        self.relaxations[1,:,0] = upper_slope
        self.relaxations[0,:,1] = lower_const
        self.relaxations[1,:,1] = upper_const


    def derive_const(self):
        size = self.input_size
        const = torch.zeros(
                size, dtype=self.config.PRECISION, device=self.config.DEVICE
            )
    ...
    def get_relu_slope(
           ...
           # out_flag: torch.tensor=None,
            ...
            if slope_type  == 'lower':
                ...
                sl[self.get_inactive_flag().flatten()] = 0.0
                idxs = self.get_unstable_flag().flatten()
                #idxs = node.get_unstable_flag().flatten()
                if slopes is None:
                    slopes = self.get_lower_relaxation_slope()
                ...
                idxs = self.get_unstable_flag().flatten()
                lower = self.from_node[0].bounds.lower.flatten()[idxs] 
                upper = self.from_node[0].bounds.upper.flatten()[idxs]
                sl[idxs] =  upper / (upper - lower)
                sl[self.get_active_flag().flatten()] = 1
...
class Relu(Node):
    ...
    def get_unstable_flag(self) -> torch.Tensor:
        ...
        self.non_lin_indices = torch.flatten(torch.nonzero(self.unstable_flag))
        if self.first_calc_bounds_flag is True:
            self.first_calc_bounds_flag = False
            self.origin_non_lin_indices = self.non_lin_indices.clone()
        if self.forced_states > 0:
            x = torch.logical_or(
                self.unstable_flag,
                torch.Tensor(
            ...
            if sum(self.unstable_flag) < sum(x):
    ...
    def get_lower_relaxation_slope(self) -> torch.Tensor:
        ...
        self.set_lower_relaxation_slope(approx=self.config.SIP.RELU_APPROXIMATION)
        return self._lower_relaxation_slope
