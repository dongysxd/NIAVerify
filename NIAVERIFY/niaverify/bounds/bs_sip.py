...
class BSSIP:
    ...
    def _derive_symb_eq(self, node: Node):
        ...
        out_flag = self._get_out_prop_flag(node)
        # if out_flag != None:
        #     Node.get_next_relu().origin_non_lin_indices = torch.flatten(torch.nonzero(self.unstable_flag))
    ...
    def back_substitution(
        ...
        equation, i_flag = self._back_substitution(
            equation, node, node, bound, i_flag, slopes, os_sip,calc_last_node
        )

        if equation is None or calc_last_node is True:
            return None, i_flag
    ...
    def _back_substitution(
        ...
        ####A
        elif calc_last_node is True:
            equation, i_flag = self._back_substitution_linear(
                equation, base_node, cur_node, bound, i_flag, slopes, os_sip, calc_last_node
            )
        ####A
        else:
            equation, i_flag = self._back_substitution_linear(
...
#A
        if (type(cur_node) in [Relu, Flatten, Unsqueeze, Reshape] )and (base_node is not self.prob.nn.tail or calc_last_node):
            #base_node.intermediate_bounds
            node_relu = self.prob.nn.tail  if calc_last_node is True else base_node.to_node[0]
            key1 = cur_node.depth
            key2 = bound
            bounds_symbolic_post = torch.cat((equation.matrix.clone().cpu(), equation.const.clone().cpu().unsqueeze(1)), dim=1)
            #bounds_symbolic_post =torch.cat((equation.matrix.clone().cpu(),equation.const.clone().cpu()),dim = 1) 
            if key1 not in node_relu.intermediate_bounds.keys():
                #pass
                node_relu.intermediate_bounds[key1] = {key2: bounds_symbolic_post.clone().cpu()}
            else:
                if key2 not in node_relu.intermediate_bounds[key1].keys():
                    #pass
                    node_relu.intermediate_bounds[key1][key2] = bounds_symbolic_post.clone().cpu()
                else:
                    node_relu.intermediate_bounds[key1][key2] = \
                        torch.cat((node_relu.intermediate_bounds[key1][key2],
                                    bounds_symbolic_post.clone().cpu()),
                                    dim=0)
...
#AD
        ...
        if type(cur_node) in [Relu, Flatten, Unsqueeze, Reshape]:
            ...
            #if non_linear_cond is True and cur_node.depth != base_node.depth:
        ...
        if cur_node.from_node[0].is_sip_branching_node() is True:
            equation, i_flag = self._back_substitution_branching(
                equation, base_node, cur_node.from_node[0], bound, i_flag, slopes, os_sip,calc_last_node
            )
        else:
            equation, i_flag = self._back_substitution_linear(
                equation, base_node, cur_node.from_node[0], bound, i_flag, slopes, os_sip,calc_last_node
